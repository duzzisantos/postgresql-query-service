...
class Updaters:
    def updateOne(table: str, primary_column: str, secondary_column: str, set_value: str, where_value: str, cursor: dict):
        return cursor.execute("UPDATE %s SET %s = %s WHERE %s =  %s", (table, primary_column, set_value, secondary_column, where_value ))
    
  
    def updateMany(table: str, primary_columns: list[str], secondary_column: str, set_values: list[str], where_value: str, cursor: dict):
