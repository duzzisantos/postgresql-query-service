...
class Updaters:
    def updateOne(table, primary_column: str, secondary_column: str, set_value: str, where_value: str, cursor: function):
        return cursor.execute("UPDATE %s SET %s = %s WHERE %s =  %s", (table, primary_column, set_value, secondary_column, where_value ))
    
  
    def updateMany(table: str, primary_columns: list[str], secondary_column: str, set_values: list[str | int | bool | date], where_value: str, cursor: function):
